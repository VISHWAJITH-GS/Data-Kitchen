...
export function EncodingStep({ schema }: { schema?: SchemaMetadata | null }) {
  const { state, dispatch } = useWizard();
  const options = [
    { id: 'onehot', title: 'One-Hot Encoding', desc: ' Best for nominal categories with no inherent order.' },
    { id: 'label', title: 'Label / Ordinal Encoding', desc: ' Best for ordinal categories with meaningful order.' },
    { id: 'target', title: 'Target Encoding', desc: ' Replaces categories with the mean target value.' },
  ];
...
export function OutlierDetectionStep({ schema }: { schema?: SchemaMetadata | null }) {
  const { state, dispatch } = useWizard();
  const options = [
    { id: 'remove', title: 'Remove Outliers', desc: ' Simply drop rows that contain statistical outliers.' },
    { id: 'cap', title: 'Capping / Winsorization', desc: ' Cap extreme values to a specific percentile threshold.' },
...
export function DimensionalityReductionStep() {
  const { state, dispatch } = useWizard();
  const options = [
    { id: 'pca', title: 'PCA (Principal Component Analysis)', desc: 'Linear dimensionality reduction.' },
    { id: 'tsne', title: 't-SNE / UMAP', desc: 'Non-linear reduction, generally better for visualization.' },
  ];
...
export function ImbalancedDataStep() {
  ...
  const options = [
    ...
    { id: 'undersample', title: 'Random Undersampling', desc: 'Randomly drops majority class instances.' },
    { id: 'class_weights', title: 'Algorithm Class Weights', desc: 'Keep dataset as-is, but rely on cost-sensitive learning.' },
  ];
...
export function TargetVariableStep({ schema }: { schema?: SchemaMetadata | null }) {
  const { state, dispatch } = useWizard();
  const options = [
    { id: 'label_encode', title: 'Label Encode Target', desc: 'Convert string class labels to integers.' },
...
export function FinalPipelineStep({ schema }: { schema?: SchemaMetadata | null }) {
  ...
  return (
    <StepWrapper title="Download Processed Dataset" description="Your dataset has been fully processed and is ready for download." isActionable={false}>
      <div style={{ padding: '2rem', textAlign: 'center', background: 'var(--card-bg)', backdropFilter: 'blur(10px)', borderRadius: '8px', border: '1px solid var(--card-border)', color: 'var(--text-main)' }}>
        <h3 style={{ margin: '0 0 1rem 0' }}>Processing Complete!</h3>
        <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>You have successfully navigated through the Data Kitchen preprocessing wizard.</p>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem' }}>
          <button className="btn-primary" onClick={handleExportCSV}>Export Dataset</button>
        </div>
      </div>
    </StepWrapper>
  );
